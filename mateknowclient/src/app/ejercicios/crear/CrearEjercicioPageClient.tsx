...
export default function CrearEjercicioPageClient() {
    ...
    try {
      ...
      if (attachActividadId && attachClaseId && newEjercicioId) {
        try {
          ...
          const current = (all.actividades || []).find(
            (a: any) => a.id === attachActividadId
          );
          const prevIds = (current?.ejercicios || []).map((e: any) => e.id);

          await actividadService.editarActividad(
            attachClaseId,
            attachActividadId,
            { ejercicioIds: [...prevIds, newEjercicioId] }
          );

          router.push(
            `/actividades/${attachClaseId}/ver/${attachActividadId}`
          );
          return;
        } catch (err) {
